...
const LevelSelect = ({
  ...
  const hasVariants = (song.variants?.video?.length || 0) > 0;

  return (
    <div
      className="p-4 md:w-screen flex flex-col gap-16 justify-center items-center "
      onClick={(e) => {
    ...
    >
      <div className="w-5/6 md:w-1/3">
        <ContextMenu>
          ...
          <ContextMenuContent>
            {song.variants?.video.map(
              (variant: { name: string; url: string }) => (
                <ContextMenuItem key={variant.name}>
                  <Link href={`/play/${song.slug}?variant=${variant.name}`}>
                    {variant.name}
                  </Link>
                </ContextMenuItem>
              )
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-sm">注意音量 MIND VOLUME</p>
      </div>
    </div>
  );
};
